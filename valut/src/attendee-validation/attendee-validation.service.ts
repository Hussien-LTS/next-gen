import { BadRequestException, HttpException, HttpStatus, Injectable, Logger } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { RabbitMQService } from 'src/shared/rabbitmq/rabbitmq.service';
import { sendVQL } from 'src/shared/vaultReq/sendVQL';
import { v4 as uuidv4 } from 'uuid';
import { attendeeValidationMapper, mapAttendeeValidationToHCPEligibility } from './attendee-validation.mapper';
import { mapTransactionLogPayload } from 'src/shared/transactionLog/mapTransactionLogPayload';
import { TRANSACTION_DIRECTIONS } from 'src/shared/constants/transaction-log-dirction';


@Injectable()
export class AttendeeValidationService {
  private readonly logger = new Logger(AttendeeValidationService.name);
  private readonly clientId: string | undefined;
  private readonly baseUrl: string | undefined;

  constructor(
    private readonly configService: ConfigService,
    private rmqService: RabbitMQService,
  ) {
    this.baseUrl = this.configService.get<string>('VAULT_BASE_URL');
    this.clientId = this.configService.get<string>('VAULT_CLIENT_ID');
  }

  async getEventAttendeeInfo(authToken: string, attendeeId: string) {
    try {
      console.log('🚀 ~ AttendeeValidationService ~ attendeeId:', attendeeId);
      if (!authToken) {
        throw new HttpException(
          'Missing Authorization header',
          HttpStatus.BAD_REQUEST,
        );
      }
      const query = `SELECT id,event__vr.centris_id__c,account__v, event__v, object_type__vr.name__v, event__vr.status__v,event__vr.start_date__v ,event__vr.end_date__v ,object_type__vr.name__v,id,event__vr.topic__v,event__vr.country__v,event__vr.topic__v
    FROM em_attendee__v 
    WHERE (id = '${attendeeId}')`;

      const eventAttendeeRes = await sendVQL(
        this.baseUrl,
        this.clientId,
        query,
        authToken,
      );
      console.log("🚀 ~ AttendeeValidationService ~ getEventAttendeeInfo ~ eventAttendeeRes:", eventAttendeeRes)
   

      if (!eventAttendeeRes?.data[0]?.event__v)
        throw new BadRequestException('Attendee not assigned to any event');
      if (!eventAttendeeRes?.data[0]?.account__v)
        throw new BadRequestException('Invalid account');

      const accountRes = await this.getAccountInfo(
        authToken,
        eventAttendeeRes?.data[0]?.account__v,
      );

      const eventInfo = await this.getEventInfo(
        authToken,
        eventAttendeeRes?.data[0]?.event__v,
      );
      const transactionName = uuidv4();

      const attendeeFullData = {
        dataSourceAttendee: mapAttendeeValidationToHCPEligibility(
          eventAttendeeRes?.data[0],
          eventInfo[0],
        ),
        centriesAttendee: attendeeValidationMapper(
          '',
          eventAttendeeRes?.data[0],
          eventInfo[0],
          accountRes[0],
        ),
        transactionName,
      };
      const savedAttendeeDate = await this.rmqService.send(
        'attendee-validation-created',
        attendeeFullData,
      );
      console.log("🚀 ~ AttendeeValidationService ~ getEventAttendeeInfo ~ savedAttendeeDate:", savedAttendeeDate)

      let transactionLogRes;
      if (savedAttendeeDate?.response?.error) {
        throw new BadRequestException(savedAttendeeDate?.response?.message);
      } else
        transactionLogRes = await this.rmqService.send(
          'transaction-log',
          mapTransactionLogPayload({
            name: transactionName,
            direction: TRANSACTION_DIRECTIONS.VAULT_INBOUND,
            logType: 'Attendee',
            success: 'true',
          }),
        );

      const attendeeData = attendeeValidationMapper(
        transactionLogRes?.data?.Id,
        eventAttendeeRes?.data[0],
        eventInfo[0],
        accountRes[0],
      );
      console.log("🚀 ~ AttendeeValidationService ~ getEventAttendeeInfo ~ attendeeData:", attendeeData)
   
      return attendeeData;
    } catch (error) {
      await this.rmqService.emit(
        'transaction-log',
        mapTransactionLogPayload({
          name: uuidv4(),
          direction: TRANSACTION_DIRECTIONS.VAULT_INBOUND,
          logType: 'Attendee',
          success: 'false',
          errorMessage: error?.message,
        }),
      );
      return {
        isError: true,
        message: error.message,
      };
    }
  }

  async getEventInfo(authToken: string, eventId: string) {
    const query = `SELECT country__vr.name__v
    FROM em_event__v 
    WHERE (id = '${eventId}')`;
    const eventInfo = await await sendVQL(
      this.baseUrl,
      this.clientId,
      query,
      authToken,
    );
    if (!eventInfo?.data) throw new BadRequestException('Invalid event ID');
    return eventInfo?.data;
  }

  async getAccountInfo(authToken: string, accountId: string) {
    const query = `SELECT external_id__v
    FROM account__v 
    WHERE (id = '${accountId}')`;
    const accountInfo = await sendVQL(
      this.baseUrl,
      this.clientId,
      query,
      authToken,
    );
    if (!accountInfo?.data) throw new BadRequestException('Invalid event ID');
    return accountInfo?.data;
  }
}
